...
public class DubioustxnServiceImpl implements DubioustxnService {
  ...
  public void addIdCardBlackList(Integer id, Authorization auth) {
    ...
    idCardBlackList.setIdentnum(dubioustxn.getCertificateid());
    idCardBlackList.setName(dubioustxn.getCustomname());
    idCardBlackList.setIdenttype(Constants.IDCARDTYPE);
    idCardBlackList.setStatus(Constants.ACTIVE);
    idCardBlackList.setUserid(auth == null ? null : auth.getUserId());
    idCardBlackList.setUsername(auth == null ? null : auth.getUsername());
    ...
    //添加操作日志
    operatorLogService.insert(auth == null ? null : auth.getUserId(),
        auth == null ? null : auth.getUsername(), "IdCardBlackList", "加入身份证名单");
  }
  ...
  public void addCustomeBlackList(Integer id, Authorization auth) {
    ...
    customebwgList.setUserid(userId);
    customebwgList.setUsername(userName);
