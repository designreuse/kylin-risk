...
<div id="wrapper" class="divContainer">
  <div class="wrapper wrapper-content animated fadeInRight ecommerce backPageDiv">
    <div class="ibox-content m-b-sm border-bottom">
      <form role="riskGradeForm" id="riskGradeForm" action="${ctxPath}/riskGradeCust/queryPayGradeCust" method="get" data-table="queryTable" >
        <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
        <input type="hidden" name="operType" id="operType" value="${operType}"/>
        <input type="hidden" name="risktype" value="00">
        <div class="row">
        <div class="col-sm-4">
          <div class="form-group">
            <label class="control-label" for="grade">风险等级：</label>
            <dict:select dictcode="risklevel" id="grade" name="grade" nullLabel="请选择" nullOption="true" cssClass="form-control"></dict:select>
          </div>
        </div>
        ...
        <div class="col-sm-4">
          <div class="form-group">
            ...
            <input type="text" class="form-control input-sm" id="customerid" name="customerid"
                   value="${customerid}" placeholder="客户号" class="form-control">
          </div>
        </div>
      </div>
        <div class="row">
          <div class="col-sm-4">
            <div class="form-group">
              ...
              <input type="text" class="form-control input-sm" id="customername" name="customername"
                     value="${customername}" placeholder="客户名称" class="form-control">
            </div>
          </div>
          ...
          <div class="col-sm-4">
            <div class="form-group">
              ...
              <input type="text" class="form-control input-sm" id="createtime" name="createtime"
      ...
      <div class="col-lg-12">
        <div class="ibox">
          <div class="ibox-content">
            ...
            <a href="#">
              <button type="button" id ="setLow" class="btn-white btn btn-xs">
                <i class="fa fa-star-o"></i>&nbsp;&nbsp;<strong>设为低风险</strong></button>
            </a>&nbsp;&nbsp;&nbsp;&nbsp;
            <a href="#">
              <button type="button" id ="setMiddle" class="btn-white btn btn-xs">
                <i class="fa fa-star-half-empty"></i>&nbsp;&nbsp;<strong>设为中风险</strong></button>
            </a>&nbsp;&nbsp;&nbsp;&nbsp;
            <a href="#">
              <button type="button" id ="setHigh" class="btn-white btn btn-xs">
                <i class="fa fa-star"></i>&nbsp;&nbsp;<strong>设为高风险</strong></button>
            </a>
          </c:if>
            <C:if test="${operType=='CHECK'}">
              <a href="#">
                <button type="button" id ="setAgree" class="btn-white btn btn-xs">
                  <i class="fa fa-star-o"></i>&nbsp;&nbsp;<strong>审核通过</strong></button>
              </a>
              <a href="#">
                <button type="button" id ="setDisagree" class="btn-white btn btn-xs">
...
<div id="siteMeshJavaScript">
  <script type="text/javascript">
    ...
    function setRiskGrade(updateGrade){
      var selectIds="";
      var chooseGrade = $("input[name='choose']:checked");
      if (chooseGrade.length == 0) {
        bootbox.alert("请选择数据");
      }else {
        bootbox.prompt("请输入更改评级等级原因", function(result) {
          if(result==null){
            ...
            var grade = "";
            for (var i=0;i<chooseGrade.length;i++){
                if(selectIds==""){
                  grade = chooseGrade[i].getAttribute('grade');//获取选中的第一行的等级
                  selectIds+=chooseGrade[i].value;
                }else {
                  if(grade==chooseGrade[i].getAttribute('grade')){//用第一行的等级与其他行等级比较
                    selectIds+=","+chooseGrade[i].value;
                  }else{
    ...
    function getDetail(id,type) {
      $.ajaxParamBack({"url":"${ctxPath}/riskGradeCust/toAjaxGetPayRiskGradeDetail","data":"id="+id+"&operType="+$("#operType").val()});
    }
      ...
      $('form#riskGradeForm').queryTables({
        ...
        columnDefs:[
          ...
          },
          {
            targets:3,
            render:function(data, type, row, meta){
              var dataMap={"1":{text:"内部企业客户"},"2":{text:"外部企业客户"},"3":{text:"平台内部"},"4":{text:"资金托管客户"},"0":{text:"个人客户"}};
