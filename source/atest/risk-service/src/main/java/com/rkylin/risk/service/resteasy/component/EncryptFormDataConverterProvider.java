...
public class EncryptFormDataConverterProvider implements ParamConverterProvider {
  ...
  private ParamConverter matchConverter(Class rawType, Type genericType,
    ...
    if(CharSequence.class.isAssignableFrom(rawType)){
      return new StringEncryptFormDataConverter(publicKey,rawType);
    }
      ...
      if(genericType instanceof ParameterizedType){
        ...
        if(types!=null&&types.length==1){
          ...
          if(parameterType instanceof Class){
            ...
            return new MitouRequestBeanEncryptFormDataConverter(publicKey,objectMapper,parameterizedClass);
          }
        }
      }
    }
    return new ObjectEncryptFormDataConverter(publicKey,objectMapper,rawType);
  }
}
