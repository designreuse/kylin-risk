...
public class PersonMitouListener extends BaseBizImpl {
  ...
  public void updateRiskMsgListen(MitouBean event) {
    ...
    //false  代表接口调用过程中发生异常，可再次发起，validatefalse代表数据校验不通过 refuse代表该渠道不调用米投接口
    if ("false".equals(mtresult) || "validatefalse".equals(mtresult)) {
    ...
    if (StringUtils.isNotEmpty(riskmessage + mitoumsg)) {
      log.info("修改运营平台审批风险提示为：{}", riskmessage + mitoumsg);
