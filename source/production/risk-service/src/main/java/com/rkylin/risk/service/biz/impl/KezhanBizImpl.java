...
public class KezhanBizImpl implements KezhanBiz {
  ...
  public String downloadFile(PersonFactor personFactor) {
    ...
    try {
      ...
      if (imageEntityMap.get("idcardmd5check") == null
        ...
        Map ocrmap=identifyBiz.requestFaceresult(imageEntityMap, personFactor);
        String riskmessage = ocrmap.get("msg").toString();
  ...
  public Map<String, Object> unzip(byte[] zipdatas) throws IOException {
    ...
    try (ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(zipdatas);
         ZipInputStream zipInputStream = new ZipInputStream(byteArrayInputStream)) {
      ZipEntry zipEntry = zipInputStream.getNextEntry();
      while (zipEntry != null) {
        byte[] encodedData;
        if (!zipEntry.isDirectory() && (zipEntry.getSize() > 0) || zipEntry.getSize() == -1) {
