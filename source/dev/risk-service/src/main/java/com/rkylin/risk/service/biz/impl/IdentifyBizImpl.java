...
import com.rkylin.facerecognition.api.service.OCRService;
...
public class IdentifyBizImpl implements IdentifyBiz {
  ...
  public Map requestFaceresult(Map<String, Object> imageEntityMap,
    ...
    map3.get("similarity");
    map.put("similarity", map3.get("similarity") == null ? null : map3.get("similarity"));
    map.put("msg", sbf.toString());
    return map;
  }

  public Map requestYSJFaceresult(Map<String, Object> imageEntityMap,
      PersonFactor factor) {
    StringBuffer sbf = new StringBuffer();
    Map map = new HashMap();
    ImageEntity imageEntity1 = (ImageEntity) imageEntityMap.get("idcard");
    ...
    //与后台公安部照片比对
    Map<String, Object> policemap =
        requestphotoMatchPolicePhoto(factor, factor.getCertificatenumber(), factor.getUsername(),
            imageEntityMap.get("idcardmd5check").toString(),imageEntity1,"idcardMatchPolicePhoto");
    map.put("similarity", policemap.get("similarity") == null ? "0" : policemap.get("similarity"));
    sbf.append(map1.get("message") == null ? "" : map1.get("message"));
    sbf.append(policemap.get("message") == null ? "" : policemap.get("message"));
    map.put("ocrmsg", sbf);
    return map;
  }
  ...
  private Map<String, Object> requestIdcerdinfo(PersonFactor factor, String idcardmd5check1,
    ...
    log.info("idcardmd5check1:" + idcardmd5check1);
    log.info("idcardmd5check2:" + idcardmd5check2);
    try {
      setAttachValues();
      ...
      if (identifyRecord != null && "success".equals(identifyRecord.getStatus())) {
        ...
        log.info("身份证正反面接口开始----------------------");
        idcardresult = ocrService.getIdCardInfo(factor.getConstid(), imageEntity1,
            imageEntity2);
        log.info("身份证正反面接口结束----------------------");
        if (idcardresult.isSuccess()) {
          log.info("ocrServiceResult:" + idcardresult.toString());
      ...
      if (idcardresult.isSuccess()) {
        ...
        if(info.getTimelimit().length()!=17){
          message.append("身份证识别的有效日期不正常;");
  ...
  private Map<String, Object> requestBankcard(PersonFactor factor, String md5check,
    ...
    StringBuffer message = new StringBuffer();
    log.info("bankmd5check:" + md5check);
    try {
      //银行卡
      setAttachValues();
      IdentifyRecord identifyRecord =
      ...
      if (identifyRecord != null && "success".equals(identifyRecord.getStatus())) {
        ...
        log.info("银行卡接口开始----------------------");
        bankresult = ocrService.getBankCardInfo(factor.getConstid(), imageEntity4);
        log.info("银行卡接口结束----------------------");
      ...
      if (bankresult.isSuccess()) {
        String ocrbankname = bankresult.getResultData().getResult().getBank_name();

        if (StringUtils.isEmpty(ocrbankname)) {
          message.append("ocr未识别出银行卡名称");
          returnMap.put("message", message);
          return returnMap;
        }

        if ("中国工商银行".contains(ocrbankname)) {
          message.append("银行卡为工商银行;");
        }
        if (StringUtils.isEmpty(factor.getBankname())) {
          message.append("rop未查询到客户的账户银行卡信息;");
        } else {
          if (factor.getBankname().contains(ocrbankname) || ocrbankname.contains(
              factor.getBankname())) {
          } else {
  ...
  private Map<String, Object> requestPersonPic(PersonFactor factor, String idcardmd5check1,
    ...
    Map<String, Object> returnMap = new HashMap<String, Object>();
    String message = "";
    log.info("personidcardmd5check1:" + idcardmd5check1);
    log.info("personidcardmd5check2:" + idcardmd5check2);
    try {
      IdentifyRecord identifyRecord =
      ...
      if (identifyRecord != null && "success".equals(identifyRecord.getStatus())) {
        ...
        returnMap.put("similarity", identifyRecord.getSimilarity());
      } else {
        //个人照片与身份证对比
        setAttachValues();
        ...
        if (personresult.isSuccess()) {
          returnMap.put("similarity", personresult.getResultData());
          IdentifyRecord record = new IdentifyRecord();
          record.setCustomerid(factor.getUserid());
          record.setStatus("success");
  ...
  public Map<String, Object> requestphotoMatchPolicePhoto(PersonFactor factor, String num,
    ...
    try {
      IdentifyRecord identifyRecord =
          getIdentifyRecord(interfacename, md5check, null, factor.getUserid());
      BaseResultVo<String> result = new BaseResultVo<String>();
      if (identifyRecord != null && "success".equals(identifyRecord.getStatus())) {
        log.info("-----个人照片与公安部照片对比记录存在");
        result.setCode(identifyRecord.getCode());
        result.setResultData(identifyRecord.getSimilarity());
        result.setSuccess(true);
        message = "个人照片与公安部照片相似度：" + identifyRecord.getSimilarity();
        returnMap.put("similarity", identifyRecord.getSimilarity());
      } else {
        log.info("个人照片与公安部照片对比开始----------------------");
        setAttachValues();
        result =
            faceRecognitionService.photoMatchPolicePhoto(factor.getConstid(), num, name,
                imageEntity);
        log.info("个人照片与公安部照片对比结束----------------------");
        log.info("policeresult:" + result.toString());
        if (result.isSuccess()) {
          IdentifyRecord record = new IdentifyRecord();
          record.setCustomerid(factor.getUserid());
          record.setStatus("success");
          ...
          record.setCheckorderid(factor.getCheckorderid());
          record.setSimilarity(result.getResultData());
          identifyRecordService.insert(record);
          message = "个人照片与公安部照片相似度：" + result.getResultData();
          returnMap.put("similarity", record.getSimilarity());
          log.info(message);
        } else {
      ...
      message = "个人照片与公安部照片比对接口异常;";
      log.info("个人照片与公安部照片比对接口异常 {}", e);
      e.printStackTrace();
    }
    returnMap.put("message", message);
    return returnMap;
  ...
  private IdentifyRecord getIdentifyRecord(String interfacename, String md5check1, String md5check2,
    ...
    } else if ("photoMatchPolicePhoto".equals(interfacename)||"idcardMatchPolicePhoto".equals(interfacename)) {
      identifyRecord.setMd5checksum1(md5check1);
    } else {
      identifyRecord.setMd5checksum1(md5check1);
