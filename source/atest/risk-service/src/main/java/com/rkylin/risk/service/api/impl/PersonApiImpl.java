...
public class PersonApiImpl implements PersonApi {
  ...
  private PersonFactor jsonToPerson(String jsonStr) {
    ...
    if (isNotEmpty(jsonStr)) {
      try {
        personFactor = jsonMapper.readValue(jsonStr, PersonFactor.class);
      } catch (JsonParseException e) {
        log.info("【个人数据】数据json串解析异常,异常信息:{}", e);
      } catch (JsonMappingException e) {
        log.info("【个人数据】数据json串映射异常,异常信息:{}", e);
      } catch (IOException e) {
        log.info("【个人数据】数据json串异常,异常信息:{}", e);
      }
    }
