...
public class OperatorBizImpl implements OperatorBiz {
  ...
  private List<String> getResources(List<Role> roles) {
  ...
  private Menu getMenu(List<Role> roles) {
    ...
    Map<Short, Menu> level1 = new HashMap<>();  // 一级菜单
    Map<Short, Menu> level2 = new HashMap<>();  // 二级菜单
    for (Role role : roles) {
      if (Constants.ACTIVE.equals(role.getStatus())) {
