...
public class LoanDealBizImpl extends BaseBizImpl implements LoanDealBiz {
  ...
  public boolean handlingLoanDeal(SimpleOrder simpleOrder) {
    ...
    // 重复订单验证
    if (order != null && (simpleOrder.getCheckorderid() == null ||
        simpleOrder.getCheckorderid().equals(order.getCheckorderid()))) {
    ...
    if (StringUtils.isNotEmpty(whiteStr)) {
      ...
      resultHandler(resultBean, null, simpleOrder,order);
      return true;
    }
    //黑名单验证
    ...
    if (StringUtils.isNotEmpty(blackStr)) {
      ...
      resultHandler(resultBean, blackStr, simpleOrder,order);
      return false;
    }
    ...
    if (merchant == null) {
      ...
      Group group = queryRuleGroup(simpleOrder.getRootInstCd(),Constants.ORDER_INFO);
      if(group!=null){
        LogicRuleBean logicRuleBean =
            calculateLoanRuleBiz.calculateLoanRule(simpleOrder, merchant);
        resultBean =logicRuleCalBiz.calLogicRule(logicRuleBean, group);
      }else{
        log.info("渠道：{},无可用规则。",simpleOrder.getRootInstCd());
      }
    ...
    //提交给米投
    if (StringUtils.isNotEmpty(simpleOrder.getCheckorderid()) && !"null".equals(
        simpleOrder.getCheckorderid()) && RuleConstants.XINGREN_CHANNEL_ID.equals(
        simpleOrder.getRootInstCd())) {
      setMitouOperateStatus(simpleOrder);
    }
    return true;
  }

  private void setMitouOperateStatus(SimpleOrder simpleOrder) {
    MitouOrderBean mitouOrderBean = new MitouOrderBean();
    mitouOrderBean.setSimpleOrder(simpleOrder);
    flowStatusEventBus.post(mitouOrderBean);
  }
  /**
   * 处理交易结果
   */
  private void resultHandler(ResultBean resultBean, String blackStr,
      SimpleOrder simpleOrder,Order oldOrder) {
    ...
    if(queryRuleGroup(simpleOrder.getRootInstCd(),Constants.ORDER_INFO)!=null){
  ...
  private void setOperateStatus(SimpleOrder simpleOrder, String status,
    ...
    if(simpleOrder.getSvcString()!=null){
      ...
      bussinesstype = js.get("businessType") == null ? "" : js.get("businessType").toString();
    }
  ...
  public MerchantLimit setMerchantLimitFromOrder(MerchantLimit merchantLimit,
    ...
    LocalDate toTime = merchantLimit.getOrderDate();
    LocalDate fromTime = DateUtil.toLocalDate(simpleOrder.getOrderDate(),"yyyyMMdd");
    //比较时间
    ...
    merchantLimit.setOrderDate(fromTime);
    merchantLimit.setOrderTime(DateUtil.toDateTime(simpleOrder.getOrderTime()));
  ...
  private MerchantLimit addMerchantLimit(SimpleOrder simpleOrder) {
    ...
    merchantLimit.setOrderDate(DateUtil.toLocalDate(simpleOrder.getOrderDate(),"yyyyMMdd"));
    merchantLimit.setOrderTime(DateUtil.toDateTime(simpleOrder.getOrderTime()));
    Amount amount = new Amount(simpleOrder.getAmount());
    merchantLimit.setDaypay(amount);
    merchantLimit.setMonthpay(amount);
