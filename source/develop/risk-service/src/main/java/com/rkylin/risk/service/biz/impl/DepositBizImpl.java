...
public class DepositBizImpl extends BaseBizImpl implements DepositBiz {
  ...
  private DepositFactor scaleDepositFactor(SimpleOrder simpleOrder,
    ...
    boolean divide = (aDouble == 0d) ? true : false;
    ...
    //提现占比异常
    depositFactor.setDepositpercent(depositExcept.multiply(100));
    //历史高风险触发
    Integer highRiskCount =
        cusFactorParamService.queryHighRiskbyUserId(simpleOrder.getUserId());
    if (highRiskCount > 0) {
      depositFactor.setHistoryrisk(true);
    } else {
      depositFactor.setHistoryrisk(false);
    }
