...
@Intercepts(value = {
    ...
    @Signature(args = {MappedStatement.class,
        Object.class}, method = "update", type = Executor.class),
...
public class RedisCachingExecutor implements Interceptor {
   ...
   *
   * 当SqlSession 执行了commit()、rollback()、close()方法， Session级别的查询语句产生的CacheKey集合以及  执行的更新语句集合应该被清空
   */
  ...
  /**
   * refresh the session cache,there are two things have to do: 1. add this session scope query logs
   * to global cache Manager 2. clear the related caches according to the update statements as
   * configured in "dependency" file 3. clear the session data
   */
  ...
  /**
   * Executor插件配置信息加载点 properties中有 "dependency" 属性来指示 配置的缓存依赖配置信息，读取文件，初始化EnhancedCacheManager
   */
