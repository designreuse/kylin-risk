...
public class BaseBizImpl{
  ...
  protected Order reSetOrder(SimpleOrder simpleOrder) {
    ...
    if(simpleOrder.getSvcString()!=null){
      ...
      order.setBusinesstype(bussinesstype);
      order.setUrlkey(js.get("urlkey") == null ? "" : js.get("urlkey").toString());
      order.setLongitude(js.get("longitude") == null ? "" : js.get("longitude").toString());
      order.setLatitude(js.get("latitude") == null ? "" : js.get("latitude").toString());
      order.setFundprovider(js.get("fundProvider") == null ? "" : js.get("fundProvider").toString());
      order.setLoanterm(js.get("loanTerm") == null ? "" : js.get("loanTerm").toString());
    }
    order.setSvcid(simpleOrder.getSvcId());
    order.setUserrelatename(simpleOrder.getUserRelateName());
  ...
  public Group queryRuleGroup(String channelCode,String serviceType){
    Group group = groupService.queryByChannelAndServiceType(channelCode, serviceType);
    return group;
  }
}
