...
public class MerchantBizImpl extends BaseBizImpl implements MerchantBiz {
  ...
  @Override public String newMerchantmsgHandle(MerchantFactor merchantFactor)
      throws IOException {
    Merchant oldMerchant = merchantService.queryByMerchantid(merchantFactor.getUserid());
    ...
    Map map = calMerchantRule(logic, merchant, merchantFactor.getConstid());
    setMerchant(merchantFactor, merchant);
    if(merchantFactor.getClasslist()!=null){
      courses = setCourseInfo(merchantFactor);
    }
    if(oldMerchant!=null){
      courseService.deleteByMerchantid(merchantFactor.getUserid());
  ...
  @Override public String queryMerchantmsgHandle(String checkorderid)
    ...
    Map map = calMerchantRule(logic, merchant,merchant.getChannelid());
    merchantService.update(merchant);
    return jsonMapper.writeValueAsString(map);
  }
  ...
  private LogicRuleBean getMerchantLogicRuleFromFactor(MerchantFactor merchantFactor) {
    ...
    logic.setTrainingincomeyear(
        amountValueOf(merchantFactor.getTrainingincomeyear(), new Amount(0)).divide(100));
  ...
  private Map calMerchantRule(LogicRuleBean logic, Merchant merchant,String channelid) {
    Map map = new LinkedHashMap();
    Group nationalHeadGroup = queryRuleGroup(channelid, Constants.NATIONAL_ORG_HEAD);
    Group nationalBranchGroup = queryRuleGroup(channelid, Constants.NATIONAL_ORG_BRANCH);
    Group largeHeadGroup = queryRuleGroup(channelid, Constants.LARGE_ORG_HEAD);
    Group largeBranchGroup = queryRuleGroup(channelid, Constants.LARGE_ORG_BRANCH);
    Group mediumGroup = queryRuleGroup(channelid, Constants.MEDIUM_ORG);

    if ("1".equals(merchant.getBranchflag())) {
      ResultBean groupHeadResult = null;
      //全国集团类培训机构总部验证
      if (nationalHeadGroup!=null) {
        groupHeadResult = logicRuleCalBiz.calLogicRule(logic,nationalHeadGroup);
      }
      if (groupHeadResult != null && StringUtils.isEmpty(groupHeadResult.getOffMsg())) {
        ...
        if (largeHeadGroup!=null) {
          largeHeadResult = logicRuleCalBiz.calLogicRule(logic,largeHeadGroup);
        }
        if (largeHeadResult != null && StringUtils.isEmpty(largeHeadResult.getOffMsg())) {
          ...
          if (mediumGroup!=null) {
            mediumResult = logicRuleCalBiz.calLogicRule(logic,mediumGroup);
          }
      ...
      if (largeBranchGroup!=null) {
        largeBranchResult = logicRuleCalBiz.calLogicRule(logic,largeBranchGroup);
      }
      if (largeBranchResult != null && StringUtils.isEmpty(largeBranchResult.getOffMsg())) {
        ...
        if (nationalBranchGroup!=null) {
          groupBranchResult = logicRuleCalBiz.calLogicRule(logic,nationalBranchGroup);
        }
