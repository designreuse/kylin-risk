 ...
 *
 *MyBatis 全局二级缓存管理器 负责管理MyBatis内部的所有二级缓存Cache 该对象应该是 单例模式，
 * 一个MyBatis应用上下文中应该只有一个实例对象
 *
 * 该管理器维护着MyBatis所有的查询所产生的CacheKey集合，当有update 操作执行时，
 *会根据此update操作对应的StatementId，查看此StatementId是否指定了要刷新的查询缓存，然后指定此
 * @author louluan
 * @date 2014-12-5
 */
public interface RedisCachingManager {
  ...
  /**
   * 整个插件的核心，根据指定的statementId更新与之关联的二级缓存 传入的StatementId集合是由会话级别的update语句对应的StatementId，
